...
const Sidebar = () => {
  ...
  return (
    <aside className="w-80 min-h-screen bg-whiteborder-r border-gray-200 flex flex-col fixed left-0 top-0">
        ...
        <div className="flex flex-col gap-2 mt-12 p-5">
            {
                menuItems.map((item, index)=>{
                    ...
                    return (
                    <Link href={item.link} key={index} className={`flex gap-3 items-center py-2 px-4.5 rounded-lg font-medium duration-300 ${isActive ? "bg-primary/15 text-primary" : "hover:bg-gray-100"}`}>
                        <item.icon size={24}/>
                        <span>{item.name}</span>
                    </Link>
                );
